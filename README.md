# SSH SOCKS5 Proxy + HTTP Proxy

## Описание

SSH SOCKS Proxy — служба, которая создаёт постоянный SSH-туннель с динамическим пробросом портов (SOCKS5 + HTTP прокси). Позволяет маршрутизировать трафик через удалённый SSH-сервер.
...
| `HTTP_PROXY_PORT` | Порт HTTP-прокси | `7878` |
